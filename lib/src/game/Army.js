...
const AppendUnits_1 = __importDefault(require("../../old/ui/AppendUnits"));
const ArmyColors_1 = __importDefault(require("../../old/game/ArmyColors"));
...
class ArmyMenu extends skynode_1.ClosableFloatingDomNode {
    constructor(army, x, y) {
        ...
        this.append(skynode_1.el("a.menu", "Append", {
            click: () => {
                new AppendUnits_1.default(x, y);
                this.delete();
            },
        }));
...
class Arrow extends skynode_1.DomNode {
    constructor(army, direction, toX, toY) {
        ...
        this.on("mousedown", (event) => event.stopPropagation());
        this.on("click", (event) => {
            this.attack();
    ...
    async attack() {
        await DefantasyContract_1.default.attack(this.army.x, this.army.y, this.toX, this.toY);
...
class Army extends skynode_1.DomNode {
    constructor(gameBoard, x, y, armyData) {
        ...
        this.on("click", async () => {
            if (armyData.owner === await DefantasyContract_1.default.getPlayerAddress()) {
